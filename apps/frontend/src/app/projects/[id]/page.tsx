...
function KickoffPanel({ project }: { project: Project }) {
  return (
    <div className="flex flex-col gap-3">
      {project.background && (
        <InfoCard label="Background" content={project.background} />
      )}
      {project.hlr && (
        <InfoCard label="High Level Requirement" content={project.hlr} />
      )}
      {!project.background && !project.hlr && (
        ...
        >
          <p className="text-[12px]" style={{ color: 'var(--text-3)' }}>
            No background or requirements added. Edit the project to add context.
          </p>
        </div>
      )}
      <div
      ...
      >
        ...
        <p className="text-[11px]" style={{ color: 'var(--text-3)' }}>
          Run the Kick off agent from your Confluence page, then mark this step complete.
        </p>
      </div>
    </div>
  )
  ...
  return (
    <div className="flex flex-col gap-3">
      {prdRecords.length === 0 ? (
        ...
        >
          <p className="text-[12px]" style={{ color: 'var(--text-3)' }}>
            ...
            <a href="/prd-writer" className="underline" style={{ color: 'var(--accent)' }}>PRD Writer</a>{' '}
            to generate one.
          </p>
        </div>
      ) : (
        prdRecords.map((rec) => (
          ...
          >
            ...
            <div className="flex items-center gap-[8px]">
              <span
                className="text-[11px] font-medium px-[7px] py-[2px] rounded-[6px]"
                style={
                  rec.status === 'done'
            ...
            {rec.reflection && (
              <p className="text-[11px] mt-[8px] leading-[1.5]" style={{ color: 'var(--text-2)' }}>
                {rec.reflection}
              </p>
            )}
          </div>
      ...
      >
        ...
        <p className="text-[11px]" style={{ color: 'var(--text-3)' }}>
          Once the PRD is generated and reviewed, mark this step complete.
        </p>
      </div>
    </div>
  )
        ...
        linkedPrs.map((pr) => (
          ...
          >
            <div className="flex items-center justify-between mb-[4px]">
              ...
              <span
                className="text-[11px] font-medium px-[7px] py-[2px] rounded-[6px] shrink-0 ml-2"
            ...
            <div className="flex items-center gap-[8px]">
              <span
                className="text-[11px] font-medium px-[6px] py-[2px] rounded-[5px]"
                style={{ background: 'var(--surface-2)', color: 'var(--text-2)', border: '1px solid var(--border-md)' }}
              >
                {pr.repo}
              </span>
...
function InfoCard({ label, content }: { label: string; content: string }) {
  return (
    ...
    >
      <div
        className="text-[11px] font-semibold uppercase tracking-[0.05em] mb-[6px]"
        style={{ color: 'var(--text-3)' }}
      >
        {label}
      </div>
...
function DoneModal({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-[440px] rounded-[16px] p-6 mx-4"
        style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}
      >
        ...
        {openPrs.length > 0 ? (
          <>
            ...
            <div className="flex flex-col gap-2 mb-5">
              {openPrs.map((pr) => (
                <div
                  key={pr.id}
                  className="flex items-start gap-2 rounded-[8px] px-[12px] py-[8px]"
                  style={{ background: '#FAECE7', border: '1px solid #F0C4B4' }}
                >
          ...
          <p className="text-[12px] mb-5" style={{ color: 'var(--text-2)' }}>
            No open PRs linked to this project. Safe to close.
          </p>
        )}
        ...
        <div className="flex justify-end gap-2">
          <button
            type="button"
            onClick={onCancel}
            className="px-4 py-[8px] rounded-[8px] text-[12px] font-medium transition-colors"
            style={{ background: 'var(--surface-2)', color: 'var(--text-2)', border: '1px solid var(--border-md)' }}
          >
            Cancel
          </button>
          <button
            type="button"
            onClick={onConfirm}
            className="px-4 py-[8px] rounded-[8px] text-[12px] font-semibold text-white transition-opacity hover:opacity-90"
            style={{ background: openPrs.length > 0 ? '#993C1D' : 'var(--teal)' }}
          >
